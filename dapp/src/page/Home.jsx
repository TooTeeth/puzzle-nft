...
function Home() {
  ...
  useEffect(() => console.log((ownedCount / tokenIds.length) * 100), [ownedCount]);

  return (
    <div className="flex flex-col items-center py-8">
      <div className="w-96 h-6 bg-gray-300 rounded-lg mb-4">
        <div className={`bg-blue-400 h-full rounded-lg`} style={{ width: `${(ownedCount / tokenIds.length) * 100}%` }}></div>
      </div>
      <div>
        NFT balance: {ownedCount} / {tokenIds.length}
      </div>
      <div className="grid grid-cols-4 w-112 mt-4">
        {tokenIds.map((v, i) => (
          <img className={`${balances[i] === 0 && "brightness-50"}`} key={v} src={`/images/${v}.jpg`} alt={v} />
        ))}
      </div>
    </div>
  );
