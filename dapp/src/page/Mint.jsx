...
function Mint() {
  ...
  return (
    <div className="py-8 flex flex-col items-center">
      <div>
        <input className="input-style" type="number" value={tokenId} onChange={(e) => setTokenId(e.target.value)} />
        <input className="input-style mx-4" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} />
        <button className={`btn-style ${!signer && "bg-gray-300 border-gray-500 cursor-not-allowed"}`} disabled={!signer} onClick={onClickMint}>
          Mint
        </button>
      </div>
      <div className="mt-8">
